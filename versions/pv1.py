...
print(np.random.randint(low=1, high=23))
...
def model(r0, r0Arr, file, knownData, startingDay):
    ...
    populationIntermediate = [[0 for i in range(8)] for j in range(totalPopulation-len(knownData))] 
    for i in range(0, len(populationIntermediate)):
        # print(i)
        populationIntermediate[i][7] = 0
        ...
        for j in range(newByDay[i]):
            ...
            while True:
                ...
                if keyId >= len(r0Arr):
                    x=0
                    break
            knownDataIn.append([1,i+1,i+13,x,0,0,0,42])
    # print(knownDataIn)
