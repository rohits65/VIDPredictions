...
def createCases():
    ...
    for i in range(len(newByDay)):
        for j in range(newByDay[i]):
            ...
            keyId = 0
            # while True:
            #     if rand3 < r0Arr[keyId][0] and len(newByDay)-i<12:
            #         x = r0Arr[keyId][1]
            #         x=0
            #         break
            #     keyId += 1
            ...
            #         x=0
            #         break
            knownDataIn.append([1,i+1,i+13,0,0,0,0])
    for k in range(newByDay[-1]):
        rand3 = trueRandom(0,1)
        keyId = 0
        while True:
            if rand3 < r0Arr[keyId][0]:
                ...
                break
            keyId += 1
        knownDataIn.append([1,i+2,i+14,x,0,0,0])
    # print(knownDataIn[-1])
    return knownDataIn
...
if __name__ == "__main__": 
    ...
    - [6] --> id
    '''
    p1 = multiprocessing.Process(target=multiprocessDef, args=(2,[[0.15, 1], [0.4, 3], [0.75,2], [0.9,4], [1,0]], "truePopulationWithData4-8",))
    p1.start()
    p2 = multiprocessing.Process(target=multiprocessDef, args=(2,[[0.15, 1], [0.4, 3], [0.75,2], [0.9,4], [1,0]], "truePopulationWithData4-8",))
    p2.start()
    p3 = multiprocessing.Process(target=multiprocessDef, args=(2,[[0.15, 1], [0.4, 3], [0.75,2], [0.9,4], [1,0]], "truePopulationWithData4-8",))
    p3.start()
    p4 = multiprocessing.Process(target=multiprocessDef, args=(2,[[0.15, 1], [0.4, 3], [0.75,2], [0.9,4], [1,0]], "truePopulationWithData4-8",))
    p4.start()
    p5 = multiprocessing.Process(target=multiprocessDef, args=(2,[[0.15, 1], [0.4, 3], [0.75,2], [0.9,4], [1,0]], "truePopulationWithData4-8",))
    p5.start()
