...
def model(r0, r0Arr, file, knownData, startingDay, totalPopulation):
    #Init list and time
    ...
    populationIntermediate = np.zeros([totalPopulation-len(knownData),4], dtype = int)
...
def multiprocessDef(r0, r0Arr, file, startingDay, newByDay, iterations, populationSize):
    for i in range(0,iterations):
        knownDataIn = createCases(populationSize,5,12,r0Arr, startingDay, newByDay)
        knownDataIn = np.array(knownDataIn)
        model(r0, r0Arr, file, knownDataIn, startingDay, populationSize)
    f = open(file, "a")
    f.write("\n\nEND")
...
if __name__ == "__main__": 
    ...
    multiprocessDef(2, [[0.9, 1],[1, 2]], "truePopulationWithData4-10xe", 36, [1,1,3,2,2,3,6,4,8,5,6,2,3,18,13,12,23,24,17,20,14,7,67,39,19,73,84,83,32,17,55,202,42,66,63,75,54,59,17,61,95], 5)
    dictionary = {}
    operate.operate("truePopulationWithData4-10xe", dictionary, data, 0, True)
